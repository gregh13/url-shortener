...
class TestAddUrl(unittest.TestCase):
    def test_url_no_collision(self):
        # To ensure no collision, we get a full random string for the short_url
        mock_short_url = str(uuid.uuid4())
        add_response = add_url_to_db(short_url=mock_short_url, original_url="testing_database")
        self.assertEqual(200, add_response["status_code"])

        # Delete item just added to DB
        delete_item(mock_short_url)
